...
exports.createMarchandiseNonDangereuse = async (req, res, next) => {
  const marchandiseObject = JSON.parse(JSON.stringify(req.body));

  delete marchandiseObject.id;

  db.MarchandiseNonDangereuse.create({
    ...marchandiseObject,
  })
    .then((marchandise) => {
...
exports.getOneMarchandise = async (req, res, next) => {
  let marchandise = await db.MarchandiseNonDangereuse.findOne({ where: { id: req.params.id } });
...
exports.modifyMarchandise = async (req, res, next) => {
  ...
  db.MarchandiseNonDangereuse
    ...
    .then(async () => {
      await db.MarchandiseNonDangereuse
