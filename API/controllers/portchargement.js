...
exports.getOnePort = async (req, res, next) => {
  let port = await db.PortChargement.findOne({ where: { id: req.params.id } });
...
exports.modifyPort = async (req, res, next) => {
  ...
  db.PortChargement.update(
    ...
    .then(async () => {
      await db.PortChargement.findOne({ where: { id: req.params.id } })
        .then((port) => {
          res.status(200).json(port);
        })
        .catch((error) => res.status(400).json({ error }));
